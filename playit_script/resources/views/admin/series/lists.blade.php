            ...
            <div class="card-body">
                <div class="single-table">
                    <div class="bg-light clearfix" style="margin-bottom: 20px;">
                        ...
                        @endcan
                        @can('series_add')
                        <a class="button is-success float-right" href="/admin/series/add">Add Series</a>
                        @endcan
                    </div>
                                    ...
                                    <tr>
                                        <td class="text-center">
                                            <div class="basic checkbox">
                                                <input type="checkbox" value="{{ $series->id  }}" class="hidden" tabindex="0">
                                            </div>
                                        </td>
                                        <td class="p-1">
                                        ...
                                        <td class="text-center">{{ $series->views }}</td>
                                        @can('series_update')
                                        <td class="text-center">
                                            <label class="visible checkbox">
                                                <input value="{{ $series->id  }}" type="checkbox" @if($series->visible == 1) checked @endif>
                                            </label>
                                        </td>
                                        <td class="text-center">
                                            <label class="feature checkbox">
                                                <input value="{{ $series->id  }}" type="checkbox" @if($series->feature == 1) checked @endif>
                                            </label>
                                        </td>
                                        <td class="text-center">
                                            <label class="recommended checkbox">
                                                <input value="{{ $series->id  }}" type="checkbox" @if($series->recommended == 1) checked @endif>
                                            </label>
                                        </td>
                                        @endcan
                                        @can('comments_index')
                                        <td class="text-center"><a href="{{ url('/admin/comments/'.$series->id) }}" class="button ui inverted blue button">Read All</a></td>
                                        @endcan
                                    </tr>
                                    @endforeach
                                @else
                                    <tr><td colspan="9" class="text-center">We can't find any series!</td></tr>
                                @endif
                            </tbody>
                            <tfoot>
                                <tr>
...
<script type="text/javascript">
    $(function() {
        $('.basic.checkbox input').on('change', function() {
            ...
            $('#edit')[0].href =  '/admin/series/edit/' + postId;
            ...
            $('#delete')[0].href = '/admin/series/delete/' + seriesIds;
        });
        ...
        $('.visible.checkbox input').on('change', function() {
            ...
            if(/^(0|1)$/.test(visible) && /^(\d+)$/.test(id))
            {
                ...
                var ajaxReqUrl  = '/admin/series/visible';
        ...
        $('.feature.checkbox input').on('change', function() {
            ...
            if(/^(0|1)$/.test(feature) && /^(\d+)$/.test(id))
            {
                ...
                var ajaxReqUrl  = '/admin/series/feature';
        ...
        $('.recommended.checkbox input').on('change', function() {
            ...
            if(/^(0|1)$/.test(recommended) && /^(\d+)$/.test(id))
            {
                ...
                var ajaxReqUrl  = '/admin/series/recommended';

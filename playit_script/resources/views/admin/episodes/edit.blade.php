...
<div class="main-content-inner">
    {!! Form::model($episodes, ['route' => ['update_episodes',$episodes->id],'method'=>'put','enctype' => 'multipart/form-data', 'class' => 'form episodes']) !!}
            ...
            <div class="column is-one-quarter" id="series_tmdb">
                <div class="field" id="series_id">
                    ...
                    <div class="control">
                        <div class="select is-multiple is-primary is-fullwidth">
                            ...
                            <input type="hidden" name="sid" value="{{ $episodes->series_id}}">
                        </div>
                ...
                <div class="field" id="series_seasons">
                    ...
                    <div class="control">
                        ...
                        <div class="addown is-primary is-fullwidth is-fullwidth">
                            <input class="input is-primary" type="text" name="series_seasons" placeholder="Enter Season" value="{{ $episodes->season_id}}">
                        </div>
                ...
                <div class="field" id="series_episode">
                    ...
                    <div class="control">
                        ...
                        <div class="addown is-primary is-fullwidth is-fullwidth">
                            <input class="input is-primary" type="text" name="series_episode" placeholder="Enter Episode" value="{{ $episodes->episode_id}}">
                        </div>
            ...
            <div class="column">
                <div class="columns">
                    <div class="column">
                        <div class="field image">
                            <img src="{{ $episodes->backdrop }}" alt="backdrop image" id="episode_image">
                            <input type="hidden" name="episode_image_url" value="{{ $episodes->backdrop }}">
                    ...
                    <div class="column">
                        <div class="field">
                            ...
                            <div class="control">
                                <input name="episode_name" class="input is-primary is-fullwidth" type="text" placeholder="Name" value="{{ $episodes->name }}">
                            </div>
                        </div>
                        <div class="field">
                            <label><h4>Description</h4></label>
                            <div class="control">
                                <textarea name="episode_description" class="textarea is-primary is-fullwidth" placeholder="Description">{{ $episodes->description }}</textarea>
                            </div>
                        </div>
                        <div class="field">
                            <label><h4>Air Date (YYYY-MM-DD)</h4></label>
                            <div class="control">
                                <input name="episode_airdate" class="input is-primary is-fullwidth" type="text" placeholder="Air Date" value="{{ $episodes->air_date }}">
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        @foreach($player['type'] as $key => $value)
                            <div class="control player<?php echo $i; ?>" id="player<?php echo $i; ?>" >
                                <div class="columns">
                                    ...
                                    <div class="column is-one-fifth">
                                        <div class="control">
                                            <input name="episode_player_name[]" class="input is-primary is-one-quarter" type="text" placeholder="Player Name.." value="{{$player['name'][$key]}}">
                                        </div>
                                    </div>
                                    <div class="column">
                                        <div class="control">
                                            <input name="episode_player_url[]" class="input is-primary" type="text" placeholder="Player Embed Url.." value="{{$player['url'][$key]}}">
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                @if(isset($download))
                        ...
                        @foreach($download as $key => $value)
                            <div class="control <?php echo $i; ?>" id="download<?php echo $i; ?>">
                                <div class="columns">
                                    <div class="column is-one-quarter">
                                        <div class="control">
                                            <input name="episode_download_name[]" class="input is-primary is-one-quarter" type="text" placeholder="Download Name.." value="{{ $key }}">
                                        </div>
                                    </div>
                                    <div class="column">
                                        <div class="control">
                                            <input name="episode_download_url[]" class="input is-primary" type="text" placeholder="Download Url.." value="{{ $value }}">
                                        </div>
                                    </div>
                                </div>
                            </div>
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        ...
        var $sid = $('#series_id input[name="sid"]').val();
        $.getJSON('/admin/data/get_series', function(genresData) {
            $.each(genresData, function(val, key) {
                    seriesList.addOption({ value: key.id, text: key.name + ' [ TMDB ID : '+ key.tmdb_id +' ]' });
                    seriesList.addItem($sid);
            });
        });
        ...
        function fetchSeason(id){
            ...
            $.ajax({
                url: '/admin/episodes/get_series/'+id,
                type: 'get',
                dataType: 'json',
        ...
        function fetchEpisodes(seriesSeasonID,seriesID){
            ...
            $.ajax({
                url: '/admin/episodes/get_series/'+seriesID+'/'+seriesSeasonID,
                type: 'get',
                dataType: 'json',
                success: function(data){
        ...
        function fetchEpisodesData(seriesSeasonID,seriesEpisodeID,seriesID){
            ...
            $.ajax({
                url: '/admin/episodes/get_series/'+seriesID+'/'+seriesSeasonID+'/'+seriesEpisodeID,
                type: 'get',
                dataType: 'json',
                success: function(data){
                    //title
    ...
    $(document).ready(function(){
        ...
        $("#addPlayer").click(function(){
            ...
            newPlayerDiv.after().html('<div class="columns"><div class="column is-one-fifth"><div class="select is-primary"><select name="episode_player_type[]"><option>Select Options</option><option value="embeded">Embeded</option><option value="direct">Direct Link</option><option value="stream">Stream</option></select></div></div><div class="column is-one-fifth"><input name="episode_player_name[]" class="input is-primary is-one-quarter" type="text" placeholder="Player Name.."></div><div class="column"><input name="episode_player_url[]" class="input is-primary" type="text" placeholder="Player Embed Url.."></div></div>');

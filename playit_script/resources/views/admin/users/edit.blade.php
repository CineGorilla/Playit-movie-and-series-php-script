...
<div class="main-content-inner">
    ...
    @endif
    {!! Form::model($users, ['route'=>['update_users',$users->id],'method'=>'put','class'=>'form users']) !!}
        <div class="columns" style="margin-top : 5px;">
            <div class="column">
                <div class="field">
                    <label><h4>Full Name</h4></label>
                    <div class="control">
                        <input name="user_fullname" class="input is-primary is-fullwidth" type="text" placeholder="Full Name.." value="{{ $users->name }}">
                    </div>
                </div>
                <div class="field">
                    <label><h4>Password</h4></label>
                    <div class="control">
                        {{ Form::password('user_password',[ 'class'=>'input is-primary is-fullwidth', 'placeholder'=>'Password..'])}}
                    </div>
                </div>
                <div class="field">
                    <label><h4>Email</h4></label>
                    <div class="control">
                        <input name="user_email" class="input is-primary is-fullwidth" type="email" placeholder="Email.." value="{{ $users->email }}">
                    </div>
                </div>
                <div class="field">
                    <label><h4>Role</h4></label>
                    <div class="control" id="rolevalue">
                        <select id="role" type="text" name="user_role"  placeholder="Role.." ></select>
                        <input type="hidden" name="roleid" value="{{ $users->role}}">
                    </div>
                </div>
            </div>
        </div>
        <div class="field is-grouped is-pulled-right">
            <div class="control">
                <button type="submit" class="button is-primary is-normal">Update User</button>
            </div>
            <div class="control">
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
        //Role
        var roleid = $('#rolevalue input[name="roleid"]').val();

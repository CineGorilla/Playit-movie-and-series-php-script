...
<div class="container mx-auto bg-gray-800 items-center justify-center">
    ...
    <div class="bg-black items-center justify-center">
        ...
        <div id="player-wrapper">
            ...
            <div class="ui top attached segment borderless p-0" id="player-trailer" @if($general->site_player == 'trailer') style="display: block;" @else style="display: none;" @endif>
                ...
                <div id="playerYoutube" data-plyr-provider="youtube" data-plyr-embed-id="{{ $movies->trailer }}"></div>
                <script src="https://cdn.plyr.io/3.5.6/plyr.js"></script>
                <script>
                    var playerYoutube = new Plyr('#playerYoutube', {
                        ads: { enabled: true, publisherId: '', tagUrl: '{!! base64_decode($ads->site_300x250_banner) !!}' },
                    });
                    playerYoutube.poster = '{!! $movies->backdrop !!}';
                </script>
            </div>
            {{-- Embbed  --}}
            <div class="ui top attached segment borderless p-0" id="player-embeded" @if($general->site_player == 'embeded') style="display: block;" @else style="display: none;" @endif>
                <div class="ui embed">
                    <iframe id="playerEmbeded" src="" allowfullscreen allowtransparency allow="autoplay" poster="{{ $movies->backdrop }}"></iframe>
                </div>
            ...
            <div class="ui top attached segment borderless p-0" id="player-direct" @if($general->site_player == 'direct') style="display: block;" @else style="display: none;" @endif>
                ...
                <script>
                    ...
                    playerDirect.poster = '{!! $movies->backdrop !!}';
                </script>
            ...
            <div class="ui top attached segment borderless p-0" id="player-stream" @if($general->site_player == 'stream') style="display: block;" @else style="display: none;" @endif>
                ...
                <script>
                    ...
                    playerStream.poster = '{!! $movies->backdrop !!}';
                </script>
            ...
            <div class="ui bottom attached secondary menu radiusless" id="movie-players">
                    ...
                    @endforeach
                <span>&nbsp;</span>
                <a data-url="{{ $movies->trailer }}" class="item trailer m-2 px-4 py-2 rounded" id="yttrailer">Trailer</a>
            </div>
        </div>
        {{-- Player End --}}
    </div>
        ...
        <div class="w-full flex justify-between mb-5 ">
            <div>
                <ol class="py-2 text-base flex h-auto leading-7">
                    ...
                    <li class="flex text-gray-400"><a class="" href="/movies">Movies</a><span class="iconify" data-icon="ic:outline-navigate-next" data-inline="false" style="color: white;" data-width="30" data-height="30"></span></li>
                    <li class="flex text-white"><a class="" href="/movie/{{ $movies->slug }}">{{ $movies->name }} ({{ date('Y',strtotime($movies->release_date)) }})</a></li>
                </ol>
            </div>
            <div class="py-2 text-sm md:text-base flex h-auto leading-7 text-white">
                @if(Auth::user())
                    @if(isset(Auth::user()->watchlists()->where('items_id', $movies->id)->first()->watchlist))
                        <div class="flex leading-7 cursor-pointer text-gray-400 hover:text-white duration-200" id="watchlists" data-id="{{ $movies->id }}">
        ...
        <div class="w-full flex mb-5 text-white ">
            <div class="w-full lg:w-1/4 lg:inline-flex hidden">
                <div class="block w-full pr-10 relative">
                    <img src="{{ $movies->poster }}" class="rounded">
                    <div class="absolute left-2 right-12 top-2 px-2 text-white z-50 rounded uppercase flex text-center justify-between leading-6 h-6 text-xs">
                        <div class="flex leading-6 h-6 text-xs">
                            <span class="iconify mr-1" data-icon="la:imdb" data-inline="false" data-width="25" data-height="25"></span> {{ $movies->rating }}
                        </div>
                        <div class="flex leading-5 h-5 text-xs">
                            <span class="iconify mr-1" data-icon="ion:time-outline" data-inline="false" data-width="20" data-height="20"></span> {{ $movies->duration }}min
                        </div>
                    </div>
                </div>
            </div>
            <div class="lg:w-3/4 w-full flex">
                <div class="align-center">
                    <h2 class="text-4xl mb-3">{{ $movies->name }} ({{ date('Y',strtotime($movies->release_date)) }})</h2>
                    <div class="flex divide-x-2 divide-gray-300 mb-5 uppercase text-sm">
                        <div class="pr-2">{{ date('d M, Y',strtotime($movies->release_date)) }}</div>
                        ...
                        @if(count($movies->qualities) > 0)
                            <div class="px-2">
                                @php
                                    ...
                                    $num_count = 0;
                                @endphp
                                @foreach ($movies->qualities as $singleQuality)
                    ...
                    <h3 class="text-base mb-3 text-gray-300">{{ $movies->tagline }}</h3>
                    <div>
                        <h4 class="text-xl mb-2">Overview</h4>
                        <p class="leading-6 text-gray-300 text-sm mb-5">{{ $movies->description }}</p>
                        <h4 class="text-xl mb-2">Cast</h4>
                        <p class="leading-6 text-sm mb-5">
                            @if(count($movies->actors) > 0)
                                @php
                                    $num_of_items = count($movies->actors);
                                    $num_count = 0;
                                @endphp
                                @foreach ($movies->actors as $singleActors)
                        ...
                        <p class="leading-6 text-sm mb-5">
                            @if(count($movies->directors) > 0)
                                @php
                                    $num_of_items = count($movies->directors);
                                    $num_count = 0;
                                @endphp
                                @foreach ($movies->directors as $singleDirector)
                        ...
                        <p class="leading-6 text-sm mb-5">
                            @if(count($movies->creators) > 0)
                                @php
                                    $num_of_items = count($movies->creators);
                                    $num_count = 0;
                                @endphp
                                @foreach ($movies->creators as $singleCreators)
    ...
    @if($ads->activate == 1)
        ...
        <div class=" 2xl:flex xl:flex lg:hidden md:hidden sm:hidden hidden my-4 justify-center">
            {!! base64_decode($ads->site_728x90_banner) !!}
        </div>
        @endif
        @if(isset($ads->site_468x60_banner))
        <div class="flex 2xl:hidden xl:hidden lg:flex md:flex sm:hidden hidden my-4 justify-center">
            {!!  base64_decode($ads->site_468x60_banner) !!}
        </div>
        @endif
        @if(isset($ads->site_320x100_banner))
        <div class="flex 2xl:hidden xl:hidden lg:hidden md:hidden sm:flex  my-4 justify-center">
    ...
    <div class="container mx-auto p-10 inline-block text-white">
        <h2 class="text-3xl mb-5 uppercase">Related Movies</h2>
                                    ...
                                    <div class="card-content-wrap ">
                                        @if(Auth::user())
                                            <div class="card-actions">
                                                <button class="watchlist-button-card" data-text="@if(isset(Auth::user()->watchlists()->where('items_id', $movie->id)->first()->watchlist))REMOVE FROM WATCHLIST @else ADD TO WATCHLIST @endif" id="{{ $movie->id }}">
    ...
    <div class="container bg-gray-700 mx-auto p-10 inline-block text-white" id="comment-form">
        ...
        <div class="relative w-full mb-10">
            {!! Form::open(array('url'=>'movie-comments','method'=>'POST', 'id'=>'frmComments')) !!}
                {!! Form::hidden('itemname', $movies->name, array('id' => 'itemname')) !!}
                {!! Form::hidden('itemid', $movies->id, array('id' => 'itemid')) !!}
